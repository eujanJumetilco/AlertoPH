...
function ReportsProvider({ children }) {
  ...
  const [refCounter, setRefCounter] = useState(122);
  ...
  const value = { reports, stats, addReport, getReportById, lastCreatedId };
...
/* =========================================================================
   ...
   ========================================================================= */
function DashboardScreen({ onNavigate, onOpenReport }) {
...
/* =========================================================================
   src/screens/CreateReportScreen.jsx
   ========================================================================= */
function CreateReportScreen({ onNavigate, onSubmitted }) {
...
/* =========================================================================
   ...
   ========================================================================= */
function SuccessScreen({ reportId, onNavigate, onViewReport }) {
...
/* =========================================================================
   ...
   ========================================================================= */
function ReportDetailScreen({ reportId, onNavigate }) {
  const { getReportById } = useReports();
  const report = getReportById(reportId);
  if (!report) {
    return (
...
/* =========================================================================
   Minimal stub screens for the remaining nav tabs
   ========================================================================= */
function UpdatesScreen({ onNavigate }) {
  return (
    <div className="flex flex-col h-full">
      <TopBar onMenu={() => {}} />
...
function AppShell() {
  ...
  function handleSubmitted(reportId) {
  ...
  return (
    <div className="min-h-screen w-full bg-gray-100 flex items-center justify-center py-6">
      <div className="w-full max-w-sm h-screen sm:h-[780px] bg-white sm:rounded-[2rem] sm:shadow-xl overflow-hidden flex flex-col">
        ...
        {screen === "dashboard" && (
          <DashboardScreen onNavigate={navigate} onOpenReport={handleOpenReport} />
        )}
        {screen === "create" && (
          <CreateReportScreen onNavigate={navigate} onSubmitted={handleSubmitted} />
        )}
        {screen === "success" && (
          <SuccessScreen reportId={activeReportId} onNavigate={navigate} onViewReport={handleOpenReport} />
        )}
